...
| Transformation Readiness Scan (companion funnel)   | —     | ✅     | Phases 1–6 shipped. Funnel `/scan` → lead; report permalink `/scan/r/:id` + share card + OG Worker; report email + lead drips + unsubscribe; composite Groom Score (multi-photo, self-report, prep progress, attributes, bride rubric); outfit analysis, look previews (capped), AI coach w/ Settings → Coach Knowledge. Routes under `/api/scan/*`, `/api/coach/*`; Gemini adapter `apps/api/src/lib/gemini.ts` (stub without `GEMINI_API_KEY`). Buckets: `scan-photos`. |
