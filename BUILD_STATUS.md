...
| Transformation Readiness Scan (companion funnel)        | —     | ✅     | Public `/scan` funnel → lead w/ LeadSource "Readiness Scan"; `/portal/scan` (rescan + progress + roadmap); Scans tab on `/clients/:id`; `/api/scan/{start,claim,report,photo-url}`; Gemini adapter (`apps/api/src/lib/gemini.ts`, stub when `GEMINI_API_KEY` unset); daily purge + reminders in `/api/cron/daily`. Needs private bucket `scan-photos`. |
