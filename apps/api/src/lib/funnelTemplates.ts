...
function scoreStrip(ctx: FunnelContext): string {
  ...
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;background:#f5f5f4;border-radius:10px">
    <tr>
      <td align="center" style="padding:14px 8px 4px;font-size:12px;color:#78716c">Big day readiness<br><span style="font-size:34px;font-weight:800;color:#0b7e70">${ctx.readiness}</span></td>
    </tr>
...
const scanReport: FunnelTemplate = (ctx) => {
  ...
  const html = layout(
    ...
    `
    ${p(`Your big day is in <strong>${ctx.daysToWedding} days</strong>. Here's where you stand today, and the plan to move the number.`)}
    ${scoreStrip(ctx)}
...
Your big day is in ${ctx.daysToWedding} days. Your ${ctx.product} readiness: ${ctx.readiness ?? "—"}/100.
...
Scores rate appearance only and are never a medical assessment.${unsubscribeText(ctx)}`;
  const whatsapp = `Hi ${ctx.firstName}! Your ${ctx.product} report is ready: ${ctx.readiness ?? "—"}/100 with ${ctx.daysToWedding} days to your big day. Full report + roadmap: ${ctx.reportUrl}`;
...
const dripDay3Focus: FunnelTemplate = (ctx) => {
  ...
  const html = layout(
    "This week's plan",
    `
    ${p(`Three days in. The scan found your biggest lever is <strong>${esc(ctx.focusAreas[0]?.area ?? "consistency")}</strong>, and this week's roadmap is built around it:`)}
...
const dripDay7Checkin: FunnelTemplate = (ctx) => {
  const ms = ctx.milestones.map(
    (m) => `<strong>${esc(m.title)}</strong>, due ${formatDate(m.dueDate)}`,
  );
  const subject = `One week in: what's coming up before your big day`;
  const html = layout(
    "Milestones on your calendar",
    `
    ${p(`A week since your scan, ${esc(ctx.firstName)}. Beyond the daily routine, these are the prep milestones due soon:`)}
...
${textList(ctx.milestones.map((m) => `${m.title}, due ${formatDate(m.dueDate)}`))}
...
const rescanTemplate =
  ...
  (ctx) => {
    ...
    const html = layout(
      "See your progress",
      `
      ${p(`It's been ${month === 1 ? "a month" : `${month} months`} since your ${month === 1 ? "scan" : "last check-in"}, with <strong>${ctx.daysToWedding} days</strong> to your big day.`)}
...
${month === 1 ? "A month" : `${month} months`} since your scan, ${ctx.daysToWedding} days to your big day. Take a new selfie (same angle, same light) and see your progress against your own baseline.
...
const dripT30FinalStretch: FunnelTemplate = (ctx) => {
  ...
  const html = layout(
    "The last 30 days",
    `
    ${p(`${esc(ctx.firstName)}, your big day is <strong>${ctx.daysToWedding} days</strong> away. Timing matters more than effort from here:`)}
    ${list([
      "<strong>Outfit trial</strong>: now, so alterations have two weeks.",
...
${ctx.daysToWedding} days to your big day. The final-stretch timing:
• Outfit trial: now
...
Final checklist: ${ctx.reportUrl}${unsubscribeText(ctx)}`;
  const whatsapp = `Hi ${ctx.firstName}, ${ctx.daysToWedding} days out! Outfit trial now, last facial at least 14 days before, final haircut 4-6 days before. Checklist: ${ctx.reportUrl}`;
  return { email: { subject, html, text }, whatsapp };
};
